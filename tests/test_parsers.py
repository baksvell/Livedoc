...
def test_code_signatures_builds_parameter_type_changed_kind() -> None:
    stored = CodeSignatures(
        {"m:add": "hash-old"},
        readable={"m:add": "add(a: int) -> int"},
    )
    ...
    assert len(changes) == 1
    assert changes[0].kind == "parameter_type_changed"


def test_code_signatures_keeps_general_kind_when_parameter_type_and_return_change() -> None:
    stored = CodeSignatures(
        {"m:add": "hash-old"},
        readable={"m:add": "add(a: int) -> int"},
    )
...
def test_code_signatures_builds_parameter_default_changed_kind() -> None:
    stored = CodeSignatures(
        {"m:add": "hash-old"},
        readable={"m:add": "add(a: int = 1) -> int"},
    )

    changes = stored.build_changes(
        {"m:add": "hash-new"},
        {"m:add": "add(a: int = 2) -> int"},
    )

    assert len(changes) == 1
    assert changes[0].kind == "parameter_default_changed"


def test_code_signatures_keeps_general_kind_when_parameter_default_and_return_change() -> None:
    stored = CodeSignatures(
        {"m:add": "hash-old"},
        readable={"m:add": "add(a: int = 1) -> int"},
    )
...
def test_code_signatures_builds_parameter_order_changed_kind() -> None:
    stored = CodeSignatures(
        {"m:add": "hash-old"},
        readable={"m:add": "add(a: int, b: str) -> int"},
    )

    changes = stored.build_changes(
        {"m:add": "hash-new"},
        {"m:add": "add(b: str, a: int) -> int"},
    )

    assert len(changes) == 1
    assert changes[0].kind == "parameter_order_changed"


def test_code_signatures_keeps_general_kind_when_parameter_order_and_return_change() -> None:
    stored = CodeSignatures(
        {"m:add": "hash-old"},
        readable={"m:add": "add(a: int, b: str) -> int"},
    )
